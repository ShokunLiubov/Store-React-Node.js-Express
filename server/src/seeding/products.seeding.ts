...
class seedingProducts {
	async createProducts(countProduct: any) {
		...
		const image = [
			'dolce-gabbana-the-only-one.jpeg',
			'black-opiun-2.webp',
			'black-opium.jpeg',
			'49958_f.jpeg',
			'imperatrice-3.jpeg',
		...
		try {
			for (let i = 1; i <= countProduct; i++) {
				...
				const randomClassification = Math.floor(
					Math.random() * classification.length,
				)
				...
				await Products.create({
					...
					type_of_aroma:
						typeAroma[Math.floor(Math.random() * typeAroma.length)],
