...
class statsController {
    ...
    async getOrdersByMonth(req, res, next) {
        try {

            const { year } = req.query

            const ordersByMonth = await Order.aggregate([
                { $match: { createdAt: { $gte: new Date(year, 0, 1), $lte: new Date(year, 11, 31, 23, 59, 59, 999) } } },
                {
                    $facet: {
    ...
    async getOrdersByCity(req, res, next) {
        try {
            const { year } = req.query
    ...
    async getYearsForStats(req, res, next) {
        try {
    ...
    async getProfitOverTime(req, res, next) {
        try {
            ...
            if (dataRange) {
                ...
                date = { from: new Date(year, 1, -29), to: new Date(year, 11, 31, 23, 59, 59, 999) }
            }
